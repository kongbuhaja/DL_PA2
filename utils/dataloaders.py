...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        self.albumentations = Albumentations(size=img_size) if True else None
...
class LoadRGBTImagesAndLabels(LoadImagesAndLabels):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # TODO: MixUp augmentation
            if random.random() < hyp["mixup"] or True:
                imgs, labels = mixup_rgbt(imgs, labels, *self.load_mosaic(random.choice(self.indices)))
        ###########################        
            ...
            for ii, (img, (h0, w0), (h, w)) in enumerate(zip(imgs, hw0s, hw1s)):
                ...
                if self.augment:
                    # raise NotImplementedError('Please make data augmentation work!')
                    if M is None:
                        M, perspective, s, border = get_aug_param(img,
                                            degrees=hyp["degrees"],
                                            translate=hyp["translate"],
                                            scale=hyp["scale"],
                                            shear=hyp["shear"],
                                            perspective=hyp["perspective"])
                    img, labels = perspective_transform(
                        img,
                        labels,
                        M=M,
        ...
        nl = len(labels)  # number of labels
        if nl:
            labels[:, 1:5] = xyxy2xywhn(labels[:, 1:5], w=imgs[1].shape[1], h=imgs[1].shape[0], clip=True, eps=1e-3)

        ####################################
        if self.augment:
            f_ud, f_lr =  random.random() < hyp["flipud"],  random.random() < hyp["fliplr"]
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            # place img in img4
            if i == 0:  # top left
                rgb4 = np.full((s * 2, s * 2, imgs[0].shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                t4 = np.full((s * 2, s * 2, imgs[1].shape[2]), 114, dtype=np.uint8)
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
            elif i == 1:  # top right
                x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
            ...
            rgb4[y1a:y2a, x1a:x2a] = imgs[0][y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
            t4[y1a:y2a, x1a:x2a] = imgs[1][y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
        ...
        # Augment
        imgs4, labels4, segments4 = copy_paste_rgbt(imgs4, labels4, segments4, p=self.hyp["copy_paste"])
        M = None
        
        for ii, img4 in enumerate(imgs4):
            labels = labels4.copy()
            segments = segments4.copy()
            if M is None:
                M, perspective, s, border = get_aug_param(img4,
                                                          degrees=self.hyp["degrees"],
                                                          translate=self.hyp["translate"],
                                                          scale=self.hyp["scale"],
                                                          shear=self.hyp["shear"],
                                                          perspective=self.hyp["perspective"],
                                                          border=self.mosaic_border)
            img4, labels4 = perspective_transform(
                img4,
                labels,
                segments,
                M=M,
    ...
    def collate_fn4(batch):
        ...
        imgs, label, path, shapes, indices = zip(*batch)  # transposed
        n = len(shapes) // 4
        imgs_lwir4, imgs_vis4, label4, path4, shapes4, indices4 = [], [], [], path[:n], shapes[:n], indices[:n]
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            i *= 4
            if random.random() < 0.5:
                imgs_lwir = F.interpolate(imgs[i][0].unsqueeze(0).float(), scale_factor=2.0, mode="bilinear", align_corners=False)[
                    0
                ].type(imgs[i][0].type())
                imgs_vis = F.interpolate(imgs[i][1].unsqueeze(0).float(), scale_factor=2.0, mode="bilinear", align_corners=False)[
                    0
                ].type(imgs[i][1].type())
                lb = label[i]
            else:
                imgs_lwir = torch.cat((torch.cat((imgs[i][0], imgs[i+1][0]), 1), torch.cat((imgs[i+2][0], imgs[i+3][0]), 1)), 2)
                imgs_vis = torch.cat((torch.cat((imgs[i][1], imgs[i+1][1]), 1), torch.cat((imgs[i+2][1], imgs[i+3][1]), 1)), 2)
                lb = torch.cat((label[i], label[i + 1] + ho, label[i + 2] + wo, label[i + 3] + ho + wo), 0) * s
            imgs_lwir4.append(imgs_lwir)
            imgs_vis4.append(imgs_vis)
            label4.append(lb)

        for i, lb in enumerate(label4):
            lb[:, 0] = i  # add target image index for build_targets()

        return [torch.stack(imgs_lwir4, 0), torch.stack(imgs_vis4, 0)], torch.cat(label4, 0), path4, shapes4, indices4
